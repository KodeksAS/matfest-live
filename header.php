...
<head>
  ...
  <?php if (have_rows('main_festival_pages', 'options')) : $counter = 0; ?>
    <style>
      :root {
        <?php while (have_rows('main_festival_pages', 'options')) : the_row();
          $main_color = get_sub_field('main_color');
          $unique_id = get_sub_field('unique_id'); ?>
          
          <?php if($main_color): ?>
            --color-<?= $unique_id; ?>: var(--<?= $main_color; ?>);
          <?php endif; ?>
          <?php $counter++; ?>
        <?php endwhile; ?>
      }
...
// Check if current page is a festival page or descendant of one
// Menu is handled globally by filter in functions.php
$festival_page_id = get_current_festival_page();
$festival_logo = null;

// If we found a festival page, get its logo
if ($festival_page_id) {
  $festival_logo = get_field('festival_logo', $festival_page_id);
}
      ...
      <header id="grve-header" <?php echo implode(' ', $header_attributes); ?>>
        <div class="grve-wrapper clearfix">
          ...
          <div id="grve-main-header" class="<?php echo esc_attr($movedo_grve_header_main_class_string); ?>">
            ...
            <?php 
            // Query pages that are marked as main festival pages
            $festival_pages = new WP_Query([
              'post_type' => 'page',
              'posts_per_page' => -1,
              'meta_query' => [
                [
            ...
            if ($festival_pages->have_posts()) : ?>
              <div class="festival-tabs-wrapper">
                <ul>
                  <?php while ($festival_pages->have_posts()) : $festival_pages->the_post();
                    $page_id = get_the_ID();
                    $current_id = get_queried_object_id();
                    
                    // Set active if this page is current page or one of its ancestors
                    $is_active = false;
                    if ($current_id == $page_id) {
                      ...
                      if (is_array($ancestors) && in_array($page_id, $ancestors)) {
                        $is_active = true;
                      }
                    }
                  ...
                  ?>
                    <li class="<?= $is_active ? 'active' : ''; ?>"<?php 
                      if ($is_active && $main_color) {
                        echo ' style="--festival-color: ' . esc_attr($main_color) . '; --festival-text-color: ' . esc_attr($text_color) . ';"';
                      }
                    ?>>
                    ...
                    <div class="grve-logo grve-position-left">
                      <div class="grve-wrapper">
                        <div class="custom-logos" style="display: flex;">
                          <?php
                          // Check if we have a festival logo to display
                          if ($festival_logo && isset($festival_logo['id'])) : ?>
                            <a href="<?php echo esc_url(get_permalink($festival_page_id)); ?>">
                              <?php echo wp_get_attachment_image($festival_logo['id'], 'full', false, ['class' => 'grve-default']); ?>
                              <?php echo wp_get_attachment_image($festival_logo['id'], 'full', false, ['class' => 'grve-light']); ?>
                              <?php echo wp_get_attachment_image($festival_logo['id'], 'full', false, ['class' => 'grve-dark']); ?>
                              <?php echo wp_get_attachment_image($festival_logo['id'], 'full', false, ['class' => 'grve-sticky']); ?>
                            </a>
                          <?php else : 
                            // Default logos from custom post type
                            // args
                            $args = array(
                              'posts_per_page'    => -1,
                              'post_type'     => 'logos',
                          ...
                          ?>
                            <?php if ($the_query->have_posts()): ?>
                              ...
                              <?php while ($the_query->have_posts()) : $the_query->the_post(); ?>
                                <?php
                                $matfest_logo = get_field('matfestival_logo');
                                $matfest_link = get_field('matfestival_link');
                                if ($matfest_link):
                                  $link_url1 = $matfest_link['url'];
                                  $link_title1 = $matfest_link['title'];
                                  $link_target1 = $matfest_link['target'] ? $matfest_link['target'] : '_self';
                                endif;

                                ?>
                                <a href="<?php echo esc_url($link_url1); ?>">
                                  <?php echo wp_get_attachment_image($matfest_logo['id'], 'full', false, ['class' => 'grve-default']); ?>
                                  <?php echo wp_get_attachment_image($matfest_logo['id'], 'full', false, ['class' => 'grve-light']); ?>
                                  <?php echo wp_get_attachment_image($matfest_logo['id'], 'full', false, ['class' => 'grve-dark']); ?>
                                  <?php echo wp_get_attachment_image($matfest_logo['id'], 'full', false, ['class' => 'grve-sticky']); ?>
                                </a>
                ...
                <div class="grve-logo grve-position-left">
                  <div class="grve-wrapper">
                    <div class="custom-logos" style="display: flex;">
                      ...
                      if ($festival_logo && isset($festival_logo['id'])) : ?>
                        <a href="<?php echo esc_url(get_permalink($festival_page_id)); ?>">
                          <?php echo wp_get_attachment_image($festival_logo['id'], 'full', false, ['class' => 'grve-logo-responsive']); ?>
                        </a>
                      <?php else : 
                        // Default logos from custom post type
                        // args
                      ...
                      ?>
                        <?php if ($the_query->have_posts()): ?>
                          ...
                          <?php while ($the_query->have_posts()) : $the_query->the_post(); ?>
                            ...
                            ?>
                            <a href="<?php echo esc_url($link_url1); ?>">
                              <?php echo wp_get_attachment_image($matfest_logo['id'], 'full', false, ['class' => 'grve-logo-responsive']); ?>
                            </a>
